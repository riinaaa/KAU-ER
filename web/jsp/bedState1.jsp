<%-- 
    Document   : bedState1
    Created on : Apr 10, 2021, 2:08:54 PM
    Author     : amani
--%>
...
<<html data-wf-page="606742758fc6074e25108ff0" data-wf-site="606729d15837e87029637bf6">
    <head>
        ...
        <title>change bed state 1</title>
    </head>
</body>
 <body>
    ...
    <div data-duration-in="300" data-duration-out="100" class="tabs w-tabs">
      <div class="tabs-menu w-tab-menu">
        ...
        <%
                ...
                ResultSet result = user.getBedssall();
                if(result==null){
                    out.print("error");
              ...
              <div data-w-tab="Tab 1" class="tab-pane-tab- w-tab-pane w--tab-active">
                  <table>  
              <tbody>
                      <tr>
                          <td>bedID</td>
                      ...
                      <% while(result.next()) { %>
                      <tr>
                          <td><%= result.getInt("bedID")%></td>
                           <td><%= result.getInt("paitentID")%></td>
                           <td><%= result.getString("roomNumber")%></td>
                            <td><%= result.getString("state")%></td>
                           
                          <td><a href="bedState2.jsp?roomNumber=<%= result.getInt("roomNumber")%>&bedID=<%=result.getInt("bedID")%>">Update State</a> 
                              <a href="deleteBed.jsp?roomNumber=<%=result.getInt("roomNumber")%>&bedID=<%=result.getInt("bedID")%>">delete</a>
                          </td>
                      </tr>
                      <% } 
                      user.close();
